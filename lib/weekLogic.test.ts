...
describe('getRelevantWeekStatusLogic', () => {
  ...
  test('should identify recycling week correctly for June 6, 2025 (corrected)', () => {
    // Week containing Friday June 6, 2025 (recycling day - corrected from PDF)
    const result = getRelevantWeekStatusLogic('2025-06-03'); // Tuesday of that week
    expect('error' in result).toBe(false);
    if (!('error' in result)) {
      expect(result.weekStatus).toBe('recycling_week');
      expect(result.specialPickupDayInWeek).toBe('2025-06-06');
      expect(result.specialPickupTypeOnDate).toBe('recycling');
      expect(result.reportedWeek.startDate).toBe('2025-06-02'); // Monday
      expect(result.reportedWeek.endDate).toBe('2025-06-06'); // Friday
    }
  });
  ...
  test('should handle Thursday July 3rd recycling correctly', () => {
    ...
    if (!('error' in result)) {
      expect(result.weekStatus).toBe('recycling_week');
      expect(result.specialPickupDayInWeek).toBe('2025-07-04');
      expect(result.specialPickupTypeOnDate).toBe('recycling');
